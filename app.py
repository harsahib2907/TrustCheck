...
import secrets
import uuid
from sqlalchemy import Float, ForeignKey, create_engine, Column, Integer, String, Boolean, Text, DateTime
...
# Frontend URL (change this to your actual frontend address)
origins = [
...
app.add_middleware(
    ...
    allow_credentials=True,         # Needed if using cookies/JWT
...
def location(lat,lon):
    url = f"https://api.geoapify.com/v1/geocode/reverse?lat={lat}&lon={lon}&apiKey=4fe2472c1b414c14bd0cd38015886bff"
...
@app.post('/token')
def verify_google_token(data: dict, db: Session = Depends(get_db)):
    print("connected")
    try:
        ...
        access_token  = create_access_token(data={'sub': user_info['sub']}, expires_delta=timedelta(minutes=15))
        refresh_token, jti = create_refresh_token(data={'sub': user_info['sub']}, expires_delta=timedelta(days=30))
...
def renew_tokens(token: Annotated[str, Depends(oauth2_scheme)], db: Session = Depends(get_db)):
    ...
    try:
        ...
        if type_token == 'refresh' and jti_token == user.jti:
            refresh_token, jti = create_refresh_token(
                data={"sub": user.sub}, expires_delta=timedelta(days=30)
            )
            user.jti = jti
            db.commit()

            access_token = create_access_token(
                data={"sub": sub}, expires_delta=timedelta(minutes=15)
            )
            return {
                "access_token":  access_token,
                "refresh_token": refresh_token,
...
def distribution_qr(item:Item_batch, distributors :list ,user: Annotated[User, Depends(get_current_active_user)], db: Session = Depends(get_db)):
    if user.type_user != "company":
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Not a Company")
...
def carton_qr(item:Item_dist,cartons_count:int ,user: Annotated[User, Depends(get_current_active_user)], db: Session = Depends(get_db)):
    if user.type_user != "company":
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Not a Company")
...
def product_qr( item:Item_carton,user: Annotated[User, Depends(get_current_active_user)], db: Session = Depends(get_db)):
    if user.type_user != "company":
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Not a Company")
...
def scan_distributor(
    ...
    # Load all hashed UUIDs for this QR type and find the matching one
    # Add at the top of scan_distributor:
    if user.type_user != "distributor":
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Not a Distributor")
...
def scan_retailer(
    ...
    if user.type_user != "retailer":
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Not a Retailer")
...
def scan_customer(
    ...
    if user.type_user != "customer":
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Not a Customer")
...
def scan_supplier(
    ...
    if user.type_user != "supplier":
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Not a Supplier")
...
def list_available_lots(
    ...
    if user.type_user != "company":
        raise HTTPException(status_code=403, detail="Not a Manufacturer")
...
def list_distributor_scans(
    ...
    if user.type_user != "distributor":
        raise HTTPException(status_code=403, detail="Not a Distributor")
